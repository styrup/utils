...
$vmcdfolder  = "$($pwd.path)\$vmname-cd"
$vmisofile = "$($pwd.path)\$vmname-seed.iso"

mkdir $vmcdfolder
"instance-id: iid-local01" | Out-File -FilePath "$vmcdfolder\meta-data" -Encoding utf8 -Append
"local-hostname: cloudimg"| Out-File -FilePath "$vmcdfolder\meta-data" -Encoding utf8 -Append

$userData = Get-Content .\user-data-template
$newUSerData = $userData | %{$_.replace("HOSTNAME",$vmname)}
[IO.File]::WriteAllLines("$vmcdfolder\user-data",$newUSerData)
...
if((Get-PSDrive -Name ds -ErrorAction Ignore) -eq $null){
    New-PSDrive -Name ds -PSProvider VimDatastore -Root "\" -Location $SSD
}
Copy-DatastoreItem -Item $vmisofile -Destination ds:\cloud-init\


Import-VApp 'C:\tmp\Cloud-init\os\bionic-server-cloudimg-amd64\ubuntu-bionic-18.04-cloudimg.ovf' -Name $vmname  -VMHost (Get-VMHost) -Datastore $SSD -Location (Get-VMHost) -DiskStorageFormat Thin
# todo: her skal man bruge id eller noget.
$vm = Get-VM -Name $vmname
$vm | Get-CDDrive | Set-CDDrive -IsoPath "[SSD] cloud-init\$vmname-seed.iso" -Confirm:$false -StartConnected:$true
if($diskSizeGB -ge 10){
